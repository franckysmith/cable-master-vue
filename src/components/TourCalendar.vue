...
const months = computed(() => {
  // Mois de départ : le mois courant pour une nouvelle affaire (on ouvre sur le futur proche).
  // Si l'affaire a déjà des dates passées (édition), on recule jusqu'à la 1ʳᵉ pour les afficher.
  ...
  const list = []
  for (let k = 0; k < 12; k++) {
    const dt = new Date(base.getFullYear(), base.getMonth() + k, 1) // démarre sur le mois de base
