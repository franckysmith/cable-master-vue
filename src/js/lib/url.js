...
var url = (function() {
  ...
  url.telUrl = function(phone) {
    return (
      "tel:" +
      phone.trim().replace(/[^0-9]/g, function(nondigit, offset) {
  ...
  url.isEmail = function(email) {
    ...
    for (var i = 0; i < email.length; i++)
      if (VALID_EMAIL_CHARS_.indexOf(email.charAt(i)) == -1) return false;
  ...
  url.buildUrl = function(parts) {
    ...
    if (parts.scheme == "//") url += "//";
    else if (parts.scheme) url += parts.scheme + "://";

    if (parts.host) url += parts.host;
  ...
  url.params = function(url, params, replace) {
    ...
    for (var name in params) {
      ...
      if (qParams[name] === null) delete qParams[name];
    }
