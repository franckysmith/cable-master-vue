...
export function assert(expr, msg) {
  if (window.console && console.assert) console.assert(expr, msg);
  else if (!expr) throw new Error("Assert failed" + (msg ? ": " + msg : ""));
}
...
export var flatten = (function() {
  ...
  return function flatten(o, path) {
    path = path || "";
    if (path == "") result = {}; // discard the result filled on previous run

    for (var key in o)
      if (Object.prototype.hasOwnProperty.call(o, key))
        if (typeof o[key] == "object") flatten(o[key], nextPath(path, key));
        else result[nextPath(path, key)] = o[key];
  ...
  function insert(o) {
    ...
    if (sortby)
      a.find(function(o2, index) {
        var found =
          order == "asc" ? o[sortby] < o2[sortby] : o[sortby] > o2[sortby];
        if (found) {
          pos = index;
          return true;
        }
...
export function arrayFlip(o) {
  ...
  for (var key in o)
    if (Object.prototype.hasOwnProperty.call(o, key)) result[o[key]] = key;
...
export function arrayIntersectKey(values, keys) {
  ...
  for (var key in values)
    if (Object.prototype.hasOwnProperty.call(values, key))
      if (key in keys) result[key] = values[key];
...
export function arrayMerge(o1, o2) {
  ...
  for (var key in o1)
    if (Object.prototype.hasOwnProperty.call(o1, key)) r[key] = o1[key];

  // merge with o2
  o2 = o2 || {};
  for (key in o2)
    if (Object.prototype.hasOwnProperty.call(o2, key)) r[key] = o2[key];
...
export function arrayCombine(keys, values) {
  ...
  var r = {};
  for (var i = 0; i < keys.length; i++) r[keys[i]] = values[i];
...
export function popupWindow(url, name, options) {
  switch (arguments.length) {
    ...
    case 2:
      if (typeof name == "string" || name instanceof String) options = {};
      else {
        options = name;
        name = "_blank";
      }
  ...
  if (options.scrollbars == undefined) options.scrollbars = true;
