...
var ajax = (function() {
  ...
  function xhrSend_(request, callback) {
    ...
    if (request.postData) {
      if (request.format.output == "urlencoded") {
        ...
        if (!isEmpty(request.postData))
          // keep data==null if request.postData=={}
    ...
    xhr.onreadystatechange = function() {
      ...
      if (xhr.status != 200) {
        ...
        callback(
          handleResponse_({
            errorMsg: request.errorMsg || serverError || ajax.errors.UNKNOWN
          })
        );
        return;
      }
  ...
  ajax.send = function(request, callback) {
    ...
    if (!isEmpty(request.urlParams || {})) {
      ...
      var params = {};
      if (rParts.query) params = url.parseQuery(rParts.query);

      for (var key in request.urlParams) params[key] = request.urlParams[key]; // add new or override existing
    ...
    assert(
      r.format.input == "json" || r.format.input == "text",
      "ajax.send(): Invalid input data format"
    );
    assert(
      r.format.output == "json" || r.format.output == "urlencoded",
      "ajax.send(): Invalid output data format"
    );
